...
const globalState = {
  ...
  function getOnlyLanguagesFrom(languages) {
    return languages.map((language) => language.id.toLocaleLowerCase()).sort();
  }
  ...
  function renderDevs() {
    ...
    const renderedHTML = `
       <div>
         <h2>${filteredDevs.length} país(es) encontrado(s)</h2>
         <div class='row'>
           ${countriesToShow}
         </div>
       </div>
  ...
  function renderLanguages(languages) {
    ...
    return languages
      .map((language) => {
        return checkboxes.find((item) => item.filter === language);
      })
      .join(", ");
  }
